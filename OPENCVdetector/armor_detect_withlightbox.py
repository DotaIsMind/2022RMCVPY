...
def read_morphology_withlightbox(frame):  # read cap and morphological operation to get led binary image.
    ...
    if show_detail:
        ...
        cv2.imwrite("binary.png", dst_dilate1)
...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
        ...
        # frame = rgb_image
        # frame = cv2.resize(rgb_image, (416, 416))
        # frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        ...
        # cv2.waitKey(0)
        # cv2.waitKey(0)
        dst_dilate, frame, factor = read_morphology_withlightbox(rgb_image)
